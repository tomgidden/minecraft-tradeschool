...
public class TradingSection
{
  ...
  @Comment({"Most teach offers shown at once when a player carries several candidates.", "Vanilla's trade list scrolls, so this is about clutter rather than space."})
  public int maxTeachOffers = 0;

  @Comment({"Villager experience a learned trade grants, indexed by the villager's level.", "", "These are vanilla's own figures for enchanted-book trades: 1, 5, 10, 15 at", "levels 1-4. Vanilla has no book trade at level 5, so 30 follows its other", "level-5 trades.", "", "A flat value cannot work at both ends. Leaving level 1 takes 10 experience but", "leaving level 4 takes 100, so a number generous enough for the last promotion", "makes the first instant, and one balanced for the first cannot finish the last", "within max_uses at all.", "", "Levelling is slower here than in vanilla even so: a vanilla librarian has three", "trades per level contributing experience and a taught one may have a single", "trade. Operators who would rather match vanilla's *pace* than its numbers will", "find [2, 10, 20, 30, 30] feels closer to familiar."})
  public java.util.List<Integer> learnedTradeXp = java.util.List.of(1, 5, 10, 15, 30);

  @Comment({"What the player must supply alongside the emeralds when buying a learned", "enchanted book, as vanilla's own book trades require a blank book:", "[item, count]. Empty means emeralds alone.", "", "Books only for now. Gear would want its own material rather than paper, and", "expressing that needs a mapping from what is being sold to what it costs --", "26.1 work, not another setting beside this one."})
  public java.util.List<com.google.gson.JsonElement> bookPriceSecondary =
      java.util.List.of();

  @Comment({"How much a learned trade's price responds to reputation and demand.", "0.2 is what vanilla uses for enchanted books; its commodity trades use 0.05."})
  ...
  public java.util.Optional<net.minecraft.world.item.trading.ItemCost> bookCost()
  {
    ...
    if(cost.get(0) == null || !cost.get(0).isJsonPrimitive()) {
      Constants.LOGGER.warn(
          "book_price_secondary should be [item, count]; got {}",
          cost
      );
    ...
    if(id == null) {
      Constants.LOGGER.warn(
          "book_price_secondary names an unparseable item '{}'; "
              + "learned books will cost emeralds alone",
          cost.get(0)
      );
      return java.util.Optional.empty();
    }
    ...
    if(item.isEmpty())
      Constants.LOGGER.warn("book_price_secondary names unknown item '{}'; learned books will cost emeralds alone", id);
  ...
  @Comment({"Emerald cost bands for learned enchanted books, by the enchantment's own", "cost: common, uncommon, rare, very rare. Stated as a whole because they are", "one scale; setting a rare band below an uncommon one would invert the", "ordering."})
  public java.util.List<Integer> bookPriceBands = java.util.List.of(5, 10, 15, 20);

  @Comment({"Emeralds added per enchantment level above the first, on top of the band.", "Sharpness III in the common band costs 5 + 2 x 5."})
  public int bookPricePerLevel = 5;

  @Comment({"Least a learned trade may cost, whatever the bands work out to: [book, item].", "", "Books floor higher than gear because a book is pure knowledge -- the villager", "supplies nothing but the enchantment -- whereas a cheap tool is mostly its", "material."})
  public java.util.List<Integer> minimumPrice = java.util.List.of(5, 1);

  @Comment({"Most a learned trade may cost. 64 is a hard ceiling rather than a balance", "choice: a trade's cost is one item stack, so more would be truncated."})
  public int maximumPrice = 64;

  @Comment({"A book with several enchantments is priced on the dearest, plus each", "remaining one divided by this. 2 charges half for extras: a two-enchantment", "book should beat buying both separately, or there is no reason to want one.", "1 charges full price for every enchantment."})
  ...
  {
    return minimumPrice != null && minimumPrice.size() > index && minimumPrice.get(index) != null
        ? minimumPrice.get(index)
        : fallback;
  }

  @Comment({"Commodity trades filling profession levels that would otherwise be empty.", "", "Stripping the items a profession can be taught leaves some levels with", "nothing at all, and a villager with no trades earns no experience -- so it", "could never reach the level where teaching unlocks. These keep progression", "possible rather than being flavour.", "", "Keyed so entries can be amended, added or removed one at a time: give a key", "to change a field, or null to delete that filler entirely."})
  ...
  {
    return bookPriceBands != null && bookPriceBands.size() > index
