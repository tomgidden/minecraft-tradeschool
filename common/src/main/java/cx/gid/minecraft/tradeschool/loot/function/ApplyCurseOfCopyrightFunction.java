...
public class ApplyCurseOfCopyrightFunction extends LootItemConditionalFunction
{
  ...
  public static final MapCodec<ApplyCurseOfCopyrightFunction> CODEC = RecordCodecBuilder.mapCodec(
      instance -> commonFields(instance).and(StructureTier.CODEC.optionalFieldOf("tier").forGetter(f -> java.util.Optional.ofNullable(f.tier))).apply(instance, ApplyCurseOfCopyrightFunction::new)
  );

  @org.jetbrains.annotations.Nullable
  private final StructureTier tier;

  // 26.3 replaced the function's `conditions` list with a single optional `condition`
  // holder, so the supertype takes an Optional<Holder<…>> rather than a List. Nothing
  // here inspects it -- it is passed straight through -- so the change is purely a type.
  protected ApplyCurseOfCopyrightFunction(java.util.Optional<Holder<LootItemCondition>> condition, java.util.Optional<StructureTier> tier)
  {
    super(condition);
  ...
  protected ItemStack run(ItemStack stack, LootContext context)
  {
    try {
      ...
      Constants.LOGGER.debug("ApplyCurseOfCopyrightFunction checking {} (tier: {}, isBook: {}, isEnchanted: {})", stack.getItem(), tier, isBook, isEnchantedItem);
      ...
      Constants.debug("Applied Curse of Copyright to {} (tier: {}, probability: {}%, rolled: {})", stack.getItem(), tier, probability * 100, roll);
    }
    catch(Exception e) {
  ...
  {
    return simpleBuilder(condition -> new ApplyCurseOfCopyrightFunction(condition, java.util.Optional.ofNullable(tier)));
  }
}
