...
/// Loads the loot distribution tables bundled in the jar.
///
/// Read-only, and deliberately so: this describes which enchantments appear in which
/// structures, which is loot data rather than a setting. An operator wanting to change it
/// should ship a datapack, which overrides the resource properly and survives a mod update.
///
/// The settings an operator *does* edit live in `config/tradeschool.json`,
/// loaded separately by [cx.gid.minecraft.tradeschool.config.ConfigLoader]. This class
/// once read that file too, overlaying it onto the loot tables and generating the reference
/// from its own `global` block; both jobs have moved.
public class LootConfigLoader
{
  ...
  {
    Constants.LOGGER.info("Loading loot distribution configuration from {}", CONFIG_PATH);

    try(InputStream stream =
            LootConfigLoader.class.getResourceAsStream(CONFIG_PATH)) {
      if(stream == null) {
        Constants.LOGGER.error("Could not find loot distribution config at {}", CONFIG_PATH);
    ...
    if(config.categories == null || config.categories.isEmpty()) {
      ...
      for(Map.Entry<String, CategoryConfig> entry:
        ...
        if(category.enchantments == null || category.enchantments.isEmpty()) {
          Constants.LOGGER.warn("Category '{}' has no enchantments defined", entry.getKey());
          warnings++;
        }
        if(category.defaultWeight <= 0) {
          Constants.LOGGER.warn("Category '{}' has invalid default weight: {}", entry.getKey(), category.defaultWeight);
          warnings++;
        }
      }
    }
      ...
      for(Map.Entry<String, StructureConfig> entry:
        ...
        if(structureConfig.primaryCategories != null) {
          for(String category: structureConfig.primaryCategories) {
            if(config.categories != null && !config.categories.containsKey(category)) {
