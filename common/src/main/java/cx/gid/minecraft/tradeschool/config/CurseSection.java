...
public class CurseSection {
    ...
    @Comment({"Chance a found enchanted item carries the curse, per structure tier",
              "(low, medium, high, top). A single value applies to every tier.",
              "Empty means the curse never appears in loot."})
    ...
    public static class BookTradeSection {
        ...
        @Comment({"Fraction of eligible librarians carrying it, decided from the villager's",
                  "UUID rather than per restock, so one either always has it or never does.",
                  "0 means none do."})
        public double availability = 0.0;

        @Comment({"Emerald cost of one blank book. null prices it as an ordinary enchanted",
                  "book instead of a flat rate."})
        public Integer bookPrice = null;

        @Comment({"Villager experience per blank book sold. Only ever offered at level 5,",
                  "where there is nothing left to level up to, so this exists for operators",
                  "who lower required_level rather than to affect progression."})
        public int xp = 30;

        @Comment({"How much the price responds to reputation and demand. 0.05 matches",
                  "vanilla's commodity trades — this is stock for a shop rather than a",
                  "prize, so it should stay cheap and predictable."})
