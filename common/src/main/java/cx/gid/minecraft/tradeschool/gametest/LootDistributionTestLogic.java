...
public class LootDistributionTestLogic
{
  ...
  public static void run(GameTestHelper helper)
  {
    ...
    for(String structureId: structureIds) {
      ...
      log(helper, String.format("    Total enchanted books: %d / %d rolls (%.1f%%)", totalBooks, ROLLS, 100.0 * totalBooks / ROLLS));
      log(helper, String.format("    %-52s %6s  %6s", "Enchantment", "Count", "Rate"));
      log(helper, String.format("    %-52s %6s  %6s", "-".repeat(52), "------", "------"));

      for(Map.Entry<String, Integer> entry: sorted) {
        log(helper, String.format("    %-52s %6d  %5.2f%%", entry.getKey(), entry.getValue(), 100.0 * entry.getValue() / ROLLS));
      }
    }
    ...
    for(String structureId: structureIds) {
      ...
      for(Map.Entry<String, Integer> entry: counts.entrySet()) {
        log(helper, String.format("[CSV] %s,%s,%d,%.4f", structureId, entry.getKey(), entry.getValue(), 100.0 * entry.getValue() / ROLLS));
      }
    }
